...
namespace Linq2Ldap.Tests.Proxies {
    public class ResultPropertyValueCollectionProxyTests {
        ...
        public void Constructor_ThrowsOnNull() {
            Assert.Throws<ArgumentException>(() => new ResultPropertyValueCollectionProxy((List<object>)null));
            Assert.Throws<ArgumentException>(() => new ResultPropertyValueCollectionProxy((ResultPropertyValueCollection)null));
        }

        [Fact]
        public void Enumerator_Enumerates() {
            var m = new List<object>() { "a", "b", "c" };
            var p = new ResultPropertyValueCollectionProxy(m);
            var i = 0;
            foreach (var e in p) {
                Assert.Equal(e, m[i++]);
            }
        ...
        public void CompareTo_ReturnsListOfResults(string[] s, int[] expected) {
            ResultPropertyValueCollectionProxy p = s;
        ...
        public void LessThan_Null_Throws() {
            ResultPropertyValueCollectionProxy p2 = null;
